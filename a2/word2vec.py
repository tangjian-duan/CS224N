...
def naiveSoftmaxLossAndGradient(
    ...
    ### to integer overflow.
    y = softmax(np.dot(centerWordVec, outsideVectors.T))

    loss = -np.log(y)[outsideWordIdx]


    y[outsideWordIdx] -= 1 #y_pre - y_true
...
def negSamplingLossAndGradient(
    ...
    u_o_v_c = np.dot(outsideVectors[outsideWordIdx], centerWordVec)
    neg_sample_vec = outsideVectors[negSampleWordIndices]
    u_k_v_c = np.dot(neg_sample_vec, centerWordVec)

    loss = -np.log(sigmoid(u_o_v_c)) - np.sum(np.log(sigmoid(-u_k_v_c)))

    gradCenterVec = -(1 - sigmoid(u_o_v_c)) * outsideVectors[outsideWordIdx] +\
                    np.sum((1 - sigmoid(-u_k_v_c)[:, np.newaxis]) * neg_sample_vec, axis=0)
    ...
    for i, neg_idx in enumerate(negSampleWordIndices):
        gradOutsideVecs[neg_idx] += (1 - sigmoid(-u_k_v_c)[i]) * centerWordVec
...
def skipgram(currentCenterWord, windowSize, outsideWords, word2Ind,
    ...
    for ow in outsideWords:
        ow_Ind = word2Ind[ow]
        temp_loss, temp_gradCenterVec, temp_gradOutsideVecs = \
            word2vecLossAndGradient(centerWordVec,ow_Ind,outsideVectors,dataset)
        loss += temp_loss
        gradCenterVecs[currentCenterWordInd] += temp_gradCenterVec
