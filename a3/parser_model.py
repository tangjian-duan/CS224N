...
class ParserModel(nn.Module):
    ...
    def __init__(self, embeddings, n_features=36,
        ...
        self.embed_to_hidden = nn.Linear(in_features=n_features*self.embed_size, out_features=self.hidden_size)
        nn.init.xavier_uniform_(self.embed_to_hidden.weight, gain=1)
        self.dropout = nn.Dropout(p = dropout_prob)
        self.hidden_to_logit = nn.Linear(in_features = self.hidden_size, out_features=n_classes)
        nn.init.xavier_uniform_(self.hidden_to_logit.weight, gain=1)
        ### END YOUR CODE
    ...
    def embedding_lookup(self, t):
        ...
        x = embed.view(embed.shape[0],embed.shape[1]*embed.shape[2])
        ### END YOUR CODE
        return x
    ...
    def forward(self, t):
        ...
        h0 = self.embed_to_hidden(embeddings)
        h = nn.ReLU()(h0)
        h_drop = self.dropout(h)
        logits = self.hidden_to_logit(h_drop)
        ### END YOUR CODE
        return logits
