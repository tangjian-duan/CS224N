...
class NMT(nn.Module):
    ...
    def encode(self, source_padded: torch.Tensor, source_lengths: List[int]) -> Tuple[torch.Tensor, Tuple[torch.Tensor, torch.Tensor]]:
        ...
        X_encoded, (last_hidden, last_cell) = self.encoder(X_packed)
        enc_hiddens = pad_packed_sequence(X_encoded)[0]
    ...
    def decode(self, enc_hiddens: torch.Tensor, enc_masks: torch.Tensor,
        ...
        for y in torch.split(Y, 1, dim=0): # y (1, b, e)
            y_squeezed = torch.squeeze(y, dim=0) #(b, e)
            y_bar = torch.cat((y_squeezed,o_prev), 1) #(b, e+h)
            dec_state, o_t, e_t = self.step(y_bar, dec_state, enc_hiddens, enc_hiddens_proj, enc_masks)
    ...
    def step(self, Ybar_t: torch.Tensor,
        ...
        dec_hidden, dec_cell = dec_state[0], dec_state[1] #both (b, h)
        dec_hidden = torch.unsqueeze(dec_hidden, dim=2) #(b, h, 1)
        e_t = torch.squeeze(torch.bmm(enc_hiddens_proj,dec_hidden), dim=2) #(b, src_len)
