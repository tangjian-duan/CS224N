...
class VocabEntry(object):
    ...
    def words2charindices(self, sents):
        ...
        for sent in sents:
            ...
            for word in sent:
                char_ids = []
                char_ids.append(self.word2id['<s>'])
                for char in word:
                ...
                char_ids.append(self.word2id['</s>'])
                s_ids.append(char_ids)
            word_ids.append(s_ids)
        return word_ids
        ### END YOUR CODE
    ...
    def to_input_tensor_char(self, sents: List[List[str]], device: torch.device) -> torch.Tensor:
        ...
        padded_sents = pad_sents(sents_ids, 0)
        sents_var = torch.Tensor(padded_sents)
        sents_var.transpose(0,1)
        return sents_var
